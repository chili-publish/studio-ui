const esModules = ['@chili-publish/studio-sdk/lib/src/next', '@chili-publish/environment-client-api'].join('|');

module.exports = {
    roots: ['<rootDir>'],
    testMatch: ['**/src/tests/**/*.+(ts|tsx|js)'],
    transform: {
        [`(${esModules}).+\\.js$`]: 'babel-jest',
        '^.+\\.(ts|tsx)$': 'ts-jest',
    },
    preset: 'ts-jest',
    testEnvironment: 'jsdom',
    setupFilesAfterEnv: ['<rootDir>/src/setupTests.ts'],
    moduleNameMapper: {
        '\\.(jpg|jpeg|png|gif|eot|otf|webp|svg|ttf|woff|woff2|mp4|webm|wav|mp3|m4a|aac|oga)$':
            '<rootDir>/__mocks__/styles.mock.js',
        '\\.(css|less|scss|sass)$': '<rootDir>/__mocks__/styles.mock.js',
        '^react($|/.+)': '<rootDir>/node_modules/react$1',
        '^react-dom($|/.+)': '<rootDir>/node_modules/react-dom$1',
        '^styled-components($|/.+)': '<rootDir>/node_modules/styled-components$1',
        '^react-dnd($|/.+)': '<rootDir>/node_modules/react-dnd$1',
        '^react-dnd-html5-backend($|/.+)': '<rootDir>/node_modules/react-dnd-html5-backend$1',
        '^react-dnd-touch-backend($|/.+)': '<rootDir>/node_modules/react-dnd-touch-backend$1',
        '^dnd-core($|/.+)': '<rootDir>/node_modules/dnd-core$1',
        '^react-select($|/.+)': '<rootDir>/node_modules/react-select$1',
        '^react-datepicker($|/.+)': '<rootDir>/node_modules/react-datepicker$1',
        '^@mocks/(.*)$': '<rootDir>/__mocks__/$1',
        '^@tests/(.*)$': '<rootDir>/src/tests/$1',
        '^src/(.*)$': '<rootDir>/src/$1',
    },
    collectCoverageFrom: ['src/**', '!src/styles/specials/winter/snow/LetItSnow.tsx'],
    coverageReporters: ['lcov', 'text', 'json'],
    reporters: [
        'default',
        ['jest-junit', { suiteName: 'jest tests', outputDirectory: 'coverage', outputName: 'junit.xml' }],
    ],
    transformIgnorePatterns: [`/node_modules/(?!${esModules})`],
    testPathIgnorePatterns: ['<rootDir>/src/tests/mocks/*', '<rootDir>/automation-tests/*'],
};
