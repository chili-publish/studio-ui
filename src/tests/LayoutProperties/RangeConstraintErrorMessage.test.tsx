import {
    ConstraintMode,
    LayoutIntent,
    LayoutPropertiesType,
    MeasurementUnit,
    ResizableLayoutProperties,
} from '@chili-publish/studio-sdk';
import { screen } from '@testing-library/react';
import { renderWithProviders } from '@tests/mocks/Provider';
import RangeConstraintErrorMessage from '../../components/layoutProperties/RangeConstraintErrorMessage';

jest.mock('@chili-publish/grafx-shared-components', () => {
    const originalModule = jest.requireActual('@chili-publish/grafx-shared-components');

    return {
        __esModule: true,
        ...originalModule,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        Tooltip: ({ content, children, position, anchorId }: any) => (
            <div data-testid="tooltip" data-content={content} data-position={position} data-anchor-id={anchorId}>
                {children}
            </div>
        ),
        TooltipPosition: {
            TOP: 'top',
        },
    };
});

describe('RangeConstraintErrorMessage', () => {
    const createMockLayout = (resizableByUser: ResizableLayoutProperties): LayoutPropertiesType =>
        ({
            id: 'test-layout',
            name: 'Test Layout',
            displayName: 'Test Layout',
            width: { value: 500, isOverride: false, isReadOnly: false },
            height: { value: 800, isOverride: false, isReadOnly: false },
            intent: { value: LayoutIntent.print, isOverride: false, isReadOnly: false },
            unit: { value: MeasurementUnit.px, isOverride: false, isReadOnly: false },
            availableForUser: true,
            selectedByUser: true,
            resizableByUser,
        }) as LayoutPropertiesType;

    it('should correctly render tooltip when both min and max aspect constraints are defined and layout properties are using the unit of the layout', () => {
        const layout = createMockLayout({
            enabled: true,
            constraintMode: ConstraintMode.range,
            minAspect: { horizontal: 1, vertical: 2 },
            maxAspect: { horizontal: 3, vertical: 4 },
        });

        renderWithProviders(
            <RangeConstraintErrorMessage
                layout={layout}
                currentWidth="400 mm"
                currentHeight="200 mm"
                unit={MeasurementUnit.mm}
            />,
        );

        const tooltip = screen.getByTestId('tooltip');
        expect(tooltip).toBeInTheDocument();
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('To respect the allowed proportions (1:2 to 3:4):'),
        );
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the width is 400 mm, the height must be between 533.33 mm and 800 mm'),
        );

        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the height is 200 mm, the width must be between 100 mm and 150 mm'),
        );
    });

    it('should correctly render tooltip when both min and max aspect constraints are defined and layout properties are using custom unit', () => {
        const layout = createMockLayout({
            enabled: true,
            constraintMode: ConstraintMode.range,
            minAspect: { horizontal: 1, vertical: 3 },
            maxAspect: { horizontal: 4, vertical: 5 },
        });

        renderWithProviders(
            <RangeConstraintErrorMessage
                layout={layout}
                currentWidth="400 px"
                currentHeight="200 mm"
                unit={MeasurementUnit.mm}
            />,
        );

        const tooltip = screen.getByTestId('tooltip');
        expect(tooltip).toBeInTheDocument();
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('To respect the allowed proportions (1:3 to 4:5):'),
        );
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the width is 400 px, the height must be between 500 px and 1200 px'),
        );

        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the height is 200 mm, the width must be between 66.67 mm and 160 mm'),
        );
    });

    it('should correctly render tooltip when min aspect constraint is defined and layout properties are using the unit of the layout', () => {
        const layout = createMockLayout({
            enabled: true,
            constraintMode: ConstraintMode.range,
            minAspect: { horizontal: 1, vertical: 3 },
        });

        renderWithProviders(
            <RangeConstraintErrorMessage
                layout={layout}
                currentWidth="300 mm"
                currentHeight="900 mm"
                unit={MeasurementUnit.mm}
            />,
        );

        const tooltip = screen.getByTestId('tooltip');
        expect(tooltip).toBeInTheDocument();
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('To respect the allowed proportions (min 1:3):'),
        );
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the width is 300 mm, the height must be lower than 900 mm'),
        );

        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the height is 900 mm, the width must be higher than 300 mm'),
        );
    });

    it('should correctly render tooltip when max aspect constraint is defined and layout properties are using a custom unit', () => {
        const layout = createMockLayout({
            enabled: true,
            constraintMode: ConstraintMode.range,
            maxAspect: { horizontal: 1, vertical: 3 },
        });

        renderWithProviders(
            <RangeConstraintErrorMessage
                layout={layout}
                currentWidth="300 mm"
                currentHeight="500 cm"
                unit={MeasurementUnit.mm}
            />,
        );

        const tooltip = screen.getByTestId('tooltip');
        expect(tooltip).toBeInTheDocument();
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('To respect the allowed proportions (max 1:3):'),
        );
        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the width is 300 mm, the height must be higher than 900 mm'),
        );

        expect(tooltip).toHaveAttribute(
            'data-content',
            expect.stringContaining('If the height is 500 cm, the width must be lower than 166.67 cm'),
        );
    });
});
